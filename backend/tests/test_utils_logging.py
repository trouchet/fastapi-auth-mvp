...
@pytest.mark.parametrize("now", [datetime(2024, 6, 20), datetime(2024, 6, 21, 12, 0)])
...
@pytest.mark.parametrize("now", [datetime(2024, 6, 21, 12, 0)])
def test_doRollover_filename(now, tmp_path):
    filename = Path(tmp_path, "my_app.log")
    handler = DailyHierarchicalFileHandler(str(filename))
    with patch.object(datetime, "datetime", return_value=now):
        handler.doRollover()
    expected_filename = Path(filename.parent, now.strftime("%Y/%m/%d"), f"{filename.stem}.{now.strftime(handler.ext_style)}.{handler.ext}")
...
def test_doRollover_error_creating_folder(tmp_path, mocker):
    ...
    # Assert error handling or logging (implementation specific)